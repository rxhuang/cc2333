...
def main(event: func.EventGridEvent, context: func.Context):
    ...
    if name:
        ...
        ff = FFmpeg(executable=ff_exe, inputs={video_file: None},
                    outputs={preview_path+name+'.gif': '-y -vf "fps=1/2"'})
        ff.run()

        for filename in os.listdir(preview_path):
            if name in filename:
                with open(os.path.join(preview_path, filename), 'rb') as f:
                    blob_client = blob_service_client.\
                        get_blob_client('preview-container', filename)
